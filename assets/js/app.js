...
function renderSprints(){
  ...
  const mkStars=conf=>Array(5).fill(0)
    .map((_,i)=>`<span class="material-icons-round conf-star ${i<conf?'':'empty'}" style="font-size:14px">${i<conf?'star':'star_border'}</span>`)
    .join('');

  const mkVelocityRow=(label,value,color)=>
    `<div><div class="sprint-stat-label">${label}</div><div class="sprint-stat-val" style="color:${color}">${value||'—'}${value?' pts':''}</div></div>`;

  const mkCard=s=>{
    const st=sprintSt(s);
    const objs=s.objectives||[],dO=objs.filter(o=>o.done).length;
    return `<div class="tl-item${st.tl?' '+st.tl:''}">
      <div class="sprint-card">
        <div class="sprint-card-header">
          <div>
            ...
            <div class="sprint-card-meta">
              ...
              <div>${mkStars(s.confidence||0)}</div>
            </div>
          </div>
        ...
        <div class="sprint-card-body">
          <div class="sprint-velocity-grid${objs.length?' sprint-velocity-grid--mb':''}">
            ${mkVelocityRow('Planifié',s.velocityPlanned,'var(--primary)')}
            ${mkVelocityRow('En cours',s.velocityCurrent,'var(--warning)')}
            ${mkVelocityRow('Réalisé',s.velocityActual,s.velocityActual?'var(--success)':'var(--text3)')}
          </div>
          ${objs.length?`<div class="sprint-obj-count">${dO}/${objs.length} objectifs</div>
            ${objs.map(o=>`<div class="objective-item${!s.closed?' objective-item--clickable':''}" ${!s.closed?`onclick="toggleObj('${s.id}','${o.id}')"`:''}>
              <div class="checkbox ${o.done?'checked':''}"></div>
              <span class="objective-text${o.done?' objective-text--done':''}">${o.text}</span>
            </div>`).join('')}`:''}
        </div>
      </div>
    </div>`;
