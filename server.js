...
app.put('/api/backlog/:id', auth, adminOnly, aw(async (req, res) => {
  ...
  await pool.query(
    'UPDATE backlog SET jira_id=?,label=?,sprint_id=?,reach=?,impact=?,confidence=?,effort=?,risk=?,criticality=?,dev_validated=? WHERE id=?',
    [jiraId??'', label??'', sprintId||null, reach||0, impact||0, confidence||0, effort||0, risk||0, criticality||0, devValidated?1:0, req.params.id]
  );
  res.json({ ok: true });
}));
